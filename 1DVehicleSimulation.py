...
num_vehicles = 8
track_circumference = 100  # Circumference of the circular track
max_speed_limit = 10  # Maximum speed limit [distance/time step]
Abs_Max_Speed = 1.2*(max_speed_limit)
...
speeds = initial_speeds.copy()
acceleration_factor = 0.05*max_speed_limit #This is also the deceleration factor
min_safe_distance = 1/acceleration_factor  # Minimum safe following distance is a function of speed


# Sort the initial positions in ascending order
positions = sorted(initial_positions)

# Adjust positions iteratively to ensure distances are at least max_speed_limit
...
# Simulation loop
simulation_time = 100  # Set the simulation time
...
#This loop essentially commands each vehicle to look for the nearest vehicle and adjust speed to get closer to it. 
#There is no "train forming."
     ...
     if distance_forward < distance_backward: #if the closest vehicle is in front of you, speed up!
        if speeds[i] < max_speed_limit:
            if distance_forward[i] > (3*min_safe_distance):   
                speeds[i] += (1* acceleration_factor)
            elif distance_forward[i] > (2*min_safe_distance):
                speeds[i] += (0.5*acceleration_factor)
            elif distance_forward[i] < (2*min_safe_distance):
                speeds[i]-= acceleration_factor
            elif distance_forward[i] < min_safe_distance:
                speeds[i]=speeds[(i+1) % num_vehicles]
                speeds[i+1]+=(1* acceleration_factor)
                speeds[i]=speeds[i+1]
        elif speeds [i] > max_speed_limit:
            if distance_forward > min_safe_distance: #if you have not formed a train, then you have to go the speed limit.
                speeds[i]=max_speed_limit
            elif distance_forward < min_safe_distance: # if you have formed a train, then you can go faster.
